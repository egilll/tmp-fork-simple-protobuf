...
struct comment : sor< comment_sl, comment_ml > {};
...
struct enum_dec : seq<opt<sign>, star<digit>> {}; // NOTE: As of time of writing the original grammar has a bug -- it uses int_lit for enum_int, which uses dec_lit instead of enum_dec, thereby not allowing a plain 0 as enum value.
...
template< char Q >
struct str_lit_value : star< not_at< one< Q > >, char_value > {};
...
struct key_type : seq<
    sor< bool_type, string_type, int32_type, int64_type, sint32_type, sint64_type, uint32_type, uint64_type, fixed32_type, fixed64_type, sfixed32_type, sfixed64_type >,
    not_at< ident_other >
    > {};
...
struct extend : if_must< keyword< 'e', 'x', 't', 'e', 'n', 'd' >, sps, message_name, sps, message_body > {};

struct package : if_must< keyword< 'p', 'a', 'c', 'k', 'a', 'g', 'e' >, sps, full_ident, sps, semi > {};

struct import_option : sor< keyword< 'w', 'e', 'a', 'k' >, keyword< 'p', 'u', 'b', 'l', 'i', 'c' > >  {};
struct import : if_must< keyword< 'i', 'm', 'p', 'o', 'r', 't' >, sps, opt<import_option>, sps, str_lit, sps, semi > {};
